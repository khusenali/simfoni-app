...
function cleanupOldFenomena(now = new Date()) {
  const cutoffYear = getCutoffYear(now);
  const rows = db.prepare(`
    SELECT * FROM fenomena WHERE CAST(strftime('%Y', tanggal, '+9 hours') AS INTEGER) < ?
  `).all(cutoffYear);

  if (!rows.length) { logCleanup({ deleted: 0, cutoffYear }); return { cutoffYear, deleted: 0, archivedFiles: [] }; }
  ...
  const del = db.prepare(`DELETE FROM fenomena WHERE CAST(strftime('%Y', tanggal, '+9 hours') AS INTEGER) < ?`).run(cutoffYear);
  logCleanup({ deleted: del.changes, cutoffYear });
  return { cutoffYear, deleted: del.changes, archivedFiles };
}

function logCleanup({ deleted, cutoffYear }) {
  db.prepare(`
