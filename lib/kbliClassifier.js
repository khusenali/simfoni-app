...
function classifyEconomicSector(text, options = {}) {
  const normalized = normalizeText(text);
  const limit = options.limit || 5;
  const rules = options.rules || loadRules();
  const exclusions = options.exclusions || loadExclusions();
  ...
  return {
    sektor: top ? db.prepare('SELECT nama FROM sektor WHERE id = ?').get(top.sektorId)?.nama || null : null,
    ...
    kbli: kbli,
    matchedKeywords,
    candidates: sectors.slice(0, limit).map((x) => ({
      sektorId: x.sektorId,
      sektor: db.prepare('SELECT nama FROM sektor WHERE id = ?').get(x.sektorId)?.nama || null,
      score: x.score,
...
function saveFenomenaClassification(fenomenaId, classification) {
  if (!fenomenaId || !classification) return;

  const del = db.prepare('DELETE FROM fenomena_kbli WHERE fenomena_id = ?');
  ...
  const tx = db.transaction(() => {
    ...
    for (const item of classification.kbli || []) {
      ins.run(fenomenaId, item.id, item.score, item.rank);
    }
  });
  tx();
}
