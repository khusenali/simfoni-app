...
function getOrCreateSektor(nama) {
  if (!nama) return null;
  const existing = db.prepare("SELECT id FROM sektor WHERE nama = ?").get(nama);
  if (existing) return existing.id;
  const info = db.prepare("INSERT INTO sektor (nama) VALUES (?)").run(nama);
  return info.lastInsertRowid;
}

function getOrCreateDistrik(nama) {
  if (!nama) return null;
  const existing = db.prepare("SELECT id FROM distrik WHERE nama = ?").get(nama);
  if (existing) return existing.id;
  const info = db.prepare("INSERT INTO distrik (nama) VALUES (?)").run(nama);
  return info.lastInsertRowid;
}

function findByUrl(url) {
  if (!url) return null;
  return db.prepare("SELECT id FROM fenomena WHERE url = ?").get(url);
}

function insertFenomena(payload) {
  const sektor_id = payload.sektor_id || getOrCreateSektor(payload.sektor_nama);
  const distrik_id = payload.distrik_id || getOrCreateDistrik(payload.distrik_nama || payload.lokasi);
  ...
  try {
    ...
    if (err.code === "SQLITE_CONSTRAINT_UNIQUE" && payload.url) {
      // Race 2 proses sinkron bersamaan -- URL ini barusan kesimpen
      // proses lain pas fungsi ini masih jalan. Anggap sukses, bukan
      // error, dan gak usah lanjut nulis klasifikasi (data udah ada duluan).
      const existing = db.prepare("SELECT id FROM fenomena WHERE url = ?").get(payload.url);
  ...
  if (payload.classification) {
    ...
    saveFenomenaClassification(info.lastInsertRowid, payload.classification);
  }
...
function updateFenomena(id, payload) {
  const fields = [];
  const params = { id };
  ...
  // Kompatibilitas dengan kode lama yang masih mengirim sektor_nama.
    ...
    if (
      ...
      fields.push("sektor_id = @sektor_id");
      params.sektor_id = getOrCreateSektor(payload.sektor_nama);
  ...
  if (payload.distrik_nama !== undefined) {
    ...
    params.distrik_id = getOrCreateDistrik(payload.distrik_nama);
  }
...
function deleteFenomena(id) {
  const existing = db.prepare("SELECT id FROM fenomena WHERE id = ?").get(id);
  ...
  const tx = db.transaction(() => {
    ...
    db.prepare("UPDATE fenomena SET mirip_dengan_id = NULL, skor_kemiripan = NULL WHERE mirip_dengan_id = ?").run(id);
    db.prepare("DELETE FROM fenomena WHERE id = ?").run(id);
  });
  tx();
...
function linkIndikator(fenomenaId, indikatorIds) {
  const del = db.prepare("DELETE FROM fenomena_indikator WHERE fenomena_id = ?");
  const ins = db.prepare("INSERT OR IGNORE INTO fenomena_indikator (fenomena_id, indikator_id) VALUES (?, ?)");
  const tx = db.transaction((ids) => {
    del.run(fenomenaId);
    for (const iid of ids) ins.run(fenomenaId, iid);
  });
  tx(indikatorIds);
  return getById(fenomenaId);
}

function getSektorList() {
  return db.prepare("SELECT id, nama FROM sektor ORDER BY nama").all();
}
function getDistrikList() {
  return db.prepare("SELECT id, nama FROM distrik ORDER BY nama").all();
}
function getIndikatorList() {
  return db.prepare("SELECT id, nama, kategori FROM indikator_bps ORDER BY nama").all();
}

const { titleSimilarity } = require("./textmining");

function findSimilarFenomena(judul, tanggal, threshold = 0.5) {
  const rows = db.prepare(`
...
function markSimilar(id, miripDenganId, skorKemiripan) {
  db.prepare(`UPDATE fenomena SET mirip_dengan_id = ?, skor_kemiripan = ? WHERE id = ?`).run(miripDenganId, skorKemiripan, id);
}

function listMiripGroups({ search, sektor, distrik, status, dateFrom, dateTo, limit = 5, offset = 0 } = {}) {
  const where = [
...
function insertFenomenaWithSimilarity(payload) {
  const mirip = findSimilarFenomena(payload.judul, payload.tanggal);
  if (!mirip) return insertFenomena(payload);
  ...
  if (dataBaruLebihBaru) {
    ...
    return insertFenomena({ ...payload, miripDenganId: mirip.id, skorKemiripan: skor });
  }
  // Data yang baru masuk ini justru lebih lama -> data LAMA (yg tanggalnya lebih baru)
  // yang perlu ditandai, bukan yang baru ini.
  const newId = insertFenomena(payload);
  markSimilar(mirip.id, newId, skor);
  return newId;
}
...
function cleanupIrrelevantForSumber(namaSumber) {
  const rows = db.prepare(`
...
function getLatestTanggalForSumber(namaSumber) {
  const row = db.prepare(`
